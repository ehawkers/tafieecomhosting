...
const Cart = () => {
  ...
  const increaseValueHandler = async (index) => {
    try {
      if (cart.products[index].units == cart.products[index].productId.units.maxQuantity) {
        ...
        const { data } = await axios.put(`https://twicksback.onrender.com/api/addToCart`, {
          "userId": user._id,
  ...
  const decreaseValueHandler = async (index) => {
    try {
      const { data } = await axios.delete(`https://twicksback.onrender.com/api/dropFromCart/${user._id}/${cart.products[index].productId._id}`);
      if (data.success) {
        window.location.reload();
      } else {
        toast.error(`${data.message}`, {
